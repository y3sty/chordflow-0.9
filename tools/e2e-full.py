...
try:
    with sync_playwright() as p:
        ...
        if pg.eval_on_selector('body', 'e=>e.dataset.view') == 'stage':
        ...
        pg.click('#to-stage'); pg.wait_for_timeout(600)
        check("hybrid: stage view on", pg.eval_on_selector('body', 'e=>e.dataset.view') == 'stage')
        check("hybrid: state 2 with two filled parts", pg.get_attribute('html', 'data-state') == '2')
        check("hybrid: graph 2 nodes + edges", pg.eval_on_selector_all('.gnode', 'e=>e.length') == 2 and pg.eval_on_selector_all('.hgraph path', 'e=>e.length') >= 2)
        pg.click('#st-play'); pg.wait_for_timeout(1800)
        chord_now = (pg.text_content('#st-chord') or '').strip()
        lit = pg.eval_on_selector_all('#st-runway .ar.lit', 'e=>e.length')
        pg.screenshot(path=str(shots/"e2e-stage.png"))
        pg.click('#st-stop'); pg.wait_for_timeout(300)
        check("hybrid: playhead drives giant chord + runway", len(chord_now) > 0 and lit == 1, f"chord={chord_now}")
        pg.click('#focus-toggle'); pg.wait_for_timeout(300)
        hidden = pg.eval_on_selector('#st-horizon', 'e=>getComputedStyle(e).display') == 'none'
        pg.click('#focus-toggle'); pg.wait_for_timeout(200)
        check("hybrid: focus-scene hides map", hidden)
        pg.click('#to-constructor'); pg.wait_for_timeout(400)
