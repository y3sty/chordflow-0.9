...
- **Гибрид «Сцена-полотно» (режим исполнения)**: интерфейс растёт вместе с песней —
  при одной заполненной части это чистая сцена (гигантский аккорд, дорожка боя со свипом, рельс «дальше»,
  карта-семечко), при двух и более снизу вырастает горизонт-карта: узлы частей с мини-дорожками,
  рёбра-переходы со счётчиками ×N и петлёй цикла, плейхед бежит по узлам. Кнопки «⤢ фокус-сцена»
  (скрыть карту) и «⚙ конструктор» (возврат в режим 0.9); выбранный режим запоминается.
...
```bash
python3 tools/e2e-full.py        # 29 сценариев headless-Chromium (включая гибрид), отчёт в docs/TEST-REPORT.md
